...
3. **Hold for the opening bell** — `wait_until_open()` blocks until 08:00:00. A no-op when already past 08:00, so retries and ad-hoc runs pass straight through; capped by `MAX_PREOPEN_WAIT` so a stray early run never hangs. (`seconds_until_open()` is the pure decision half and must stay free of logging — it is called more than once per run.) Then either branch runs, never neither:
   ...
   - otherwise `open_event_page(..., patience=EVENT_LISTING_PATIENCE, preferences=...)` fetches the now-published event, re-checking for up to 20s because the publish can land a moment after 08:00:00 — failing there would cost a full ~20s re-login. `preferences` is passed only so `order_candidates()` can choose between several listings for the same day; it never affects *whether* an event matches.

   `find_event_candidates()` does the whole DOM scan in **one** `page.evaluate()` round-trip (~1.4ms) rather than an `inner_text()` call per anchor, which used to cost seconds. It returns every match; ranking and opening happen in `open_event_page()`.
