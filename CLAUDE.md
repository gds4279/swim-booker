...
8. **Verify success** — `await_verdict()` polls `registration_text()` until it says something decisive, then `classify_result()` (a pure, unit-testable function) reads it. An error phrase (`"sold out"`, `"unable to complete registration"`, `"could not be completed"`) always beats a success phrase. Note: the wizard's `"3.CONFIRMATION"` breadcrumb is rendered on every step including failure pages, and `"thank you"` appears in unrelated furniture — neither is a success indicator. If the wizard says nothing decisive, the run does **not** fail there; it falls through to the event page (see below).
9. **Independent verification** — `check_event_page()` reloads the event page and reads `purchased_slot()`. This never fails the run (the booking is already made, and retrying could double-book), and if the site names a different slot than the one selected, the site wins and the email reports the site's answer.
